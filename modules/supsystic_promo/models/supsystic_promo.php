<?php
class supsystic_promoModelNbs extends modelNbs {
	private $_apiUrl = '';
	...
	public function saveUsageStat($code, $unique = false) {
		...
		$query = 'INSERT INTO @__usage_stat SET code = "'. dbNbs::escape($code). '", visits = 1
			ON DUPLICATE KEY UPDATE visits = visits + 1';
		return dbNbs::query($query);
	}
	private function _checkUniqueStat($code) {
		$uniqueStats = get_option(NBS_CODE. '_unique_stats');
		...
		$uniqueStats[] = $code;
		update_option(NBS_CODE. '_unique_stats', $uniqueStats);
		return false;
	}
	...
	public function getUserStatsCount() {
		...
		return (int) dbNbs::get($query, 'one');
	}
	...
	public function getTourHst() {
		$hst = get_user_meta(get_current_user_id(), NBS_CODE . '-tour-hst', true);
		if(empty($hst))
			$hst = array();
		if(!isset($hst['passed']))
			$hst['passed'] = array();
	...
	public function setTourHst( $hst ) {
		update_user_meta(get_current_user_id(), NBS_CODE . '-tour-hst', $hst);
	}
	public function clearTourHst() {
		delete_user_meta(get_current_user_id(), NBS_CODE . '-tour-hst');
	}

<?php
class optionsViewNbs extends viewNbs {
	private $_news = array();
	public function getNewFeatures() {
		$res = array();
		$readmePath = NBS_DIR. 'readme.txt';
		if(file_exists($readmePath)) {
			$readmeContent = @file_get_contents($readmePath);
			if(!empty($readmeContent)) {
				$matchedData = '';
				if(preg_match('/= '. NBS_VERSION. ' =(.+)=.+=/isU', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				} elseif(preg_match('/= '. NBS_VERSION. ' =(.+)/is', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				}
    ...
    public function getAdminPage() {
		...
		if(isset($tabs[ $activeTab ]) && isset($tabs[ $activeTab ]['callback'])) {
			//frameNbs::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('tab.'. $activeTab);
		...
		frameNbs::_()->addJSVar('adminOptionsNbs', 'nbsActiveTab', $activeTab);
		...
		$this->assign('breadcrumbs', frameNbs::_()->getModule('admin_nav')->getView()->getBreadcrumbs());
	...
	public function getTabContent() {
		frameNbs::_()->addScript('admin.mainoptions', $this->getModule()->getModPath(). 'js/admin.mainoptions.js');
	...
	public function getSettingsTabContent() {
		frameNbs::_()->addScript('admin.settings', $this->getModule()->getModPath(). 'js/admin.settings.js');
		frameNbs::_()->getModule('templates')->loadJqueryUi();
		
		$options = frameNbs::_()->getModule('options')->getAll();

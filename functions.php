 ...
 */
if(!function_exists('dateToTimestampNbs')) {
    function dateToTimestampNbs($date) {
        if(empty($a))
            return false;
        $a = explode(NBS_DATE_DL, $date);
...
if(!function_exists('importNbs')) {
    function importNbs($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassNbs')) {
    function importClassNbs($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), NBS_CODE) !== false) {
					$classFile = preg_replace('/'. NBS_CODE. '/i', '', $classFile);
				}
                $path = NBS_CLASSES_DIR. $classFile. '.php';
			}
            return importNbs($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, NBS_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], NBS_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameNbs')) {
    function toeGetClassNameNbs($class) {
        $className = '';
		if(class_exists($class. strFirstUp(NBS_CODE))) 
			$className = $class. strFirstUp(NBS_CODE);
		else if(class_exists(NBS_CLASS_PREFIX. $class)) 
            $className = NBS_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjNbs')) {
    function toeCreateObjNbs($class, $params) {
        $className = toeGetClassNameNbs($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectNbs')) {
    function redirectNbs($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsNbs')) {
	function prepareParamsNbs(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsNbs::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeNbs($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeNbs')) {
	function prepareFieldCodeNbs($string) {   

	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('nbsWpTabs')) {
					$this.addClass('nbsWpTabs');
				...
				if($this.find(selector).length) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.nbsTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId
						? $this.find('.nbsTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.nbsTabContent');
					allTabsContent.hide();

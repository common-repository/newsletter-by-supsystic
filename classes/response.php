...
class responseNbs {
    ...
    public function ajaxExec($forceAjax = false) {
        $isAjax = $this->isAjax();
        $redirect = reqNbs::getVar('redirect');
	...
	public function mainRedirect($redirectUrl = '') {
		$redirectUrl = empty($redirectUrl) ? NBS_SITE_URL : $redirectUrl;
		$redirectData = array();
		if(!empty($this->errors)) {
			$redirectData['nbsErrors'] = $this->errors;
		}
		if(!empty($this->messages)) {
			$redirectData['nbsMsgs'] = $this->messages;
		}
        return redirectNbs($redirectUrl. (strpos($redirectUrl, '?') ? '&' : '?'). http_build_query($redirectData));
	}
	/**
    ...
    /**
     * Alias for responseNbs::addError, @see addError method
     */

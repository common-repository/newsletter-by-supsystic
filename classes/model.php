<?php
abstract class modelNbs extends baseObjectNbs {
	...
	public function getModule() {
		return frameNbs::_()->getModule( $this->_code );
	}
	...
	public function removeGroup($ids) {
		...
		if(!empty($ids)) {
			if(frameNbs::_()->getTable( $this->_tbl )->delete(array('additionalCondition' => 'id IN ('. implode(',', $ids). ')'))) {
	...
	public function delete($params = array()) {
		if(frameNbs::_()->getTable( $this->_tbl )->delete( $params )) {
	...
	public function insert($data) {
		...
		$id = frameNbs::_()->getTable( $this->_tbl )->insert( $data );
		if($id) {
			return $id;
		}
		$this->pushError(frameNbs::_()->getTable( $this->_tbl )->getErrors());
		return false;
	}
	...
	public function update($data, $where) {
		...
		if(frameNbs::_()->getTable( $this->_tbl )->update( $data, $where )) {
			return true;
		}
		$this->pushError(frameNbs::_()->getTable( $this->_tbl )->getErrors());
		return false;
	...
	public function getView($code = '') {
		return frameNbs::_()->getModule( $this->_code )->getController()->getView($code);
	}
}

...
class htmlNbs {
    ...
    static public $subscriptionListOptions = array();
	...
	static private function _generateCheckList($name, $params, $htmlClb, $checkedClb) {
		...
		if($params['options']) {
			...
			foreach($params['options'] as $k => $val) {
				...
				$id = 'nbsCheckRandId_'. mt_rand(1, 99999);
				$attrs .= ' id="'. $id. '"';
			...
			if($params['display'] == 'col') {
				$out = '<table class="nbsCheckTbl">'. $out. '</table>';
			}
        }
		return $out;
	}
    ...
    static public function img($src, $usePlugPath = 1, $params = array('width' => '', 'height' => '', 'attrs' => '')) {
        if($usePlugPath) $src = NBS_IMG_PATH. $src;
        return '<img src="'.$src.'" '
    ...
    static public function ajaxfile($name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '')) {
		frameNbs::_()->addScript('ajaxupload', NBS_JS_PATH. 'ajaxupload.js');
        $out = '';
        if(strpos($params['url'], 'pl='. NBS_CODE) === false)
			$params['url'] = uriNbs::_(array('baseUrl' => $params['url'], 'pl' => NBS_CODE));
    ...
    static public function statesInput($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true, 'id' => '', 'selectHtml' => '')) {
        ...
        $states = fieldAdapterNbs::getStates($params['notSelected']);
        ...
        if(empty($params['doNotAddJs'])) {
            $res .= '<script type="text/javascript">
                // <!--
                if(!toeStates.length)
                    toeStates = '. utilsNbs::jsonEncode($states). ';
    ...
    static public function countryList($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true)) {
        $params['notSelected'] = isset($params['notSelected']) ? $params['notSelected'] : true;
        $params['options'] = fieldAdapterNbs::getCountries($params['notSelected']);
		if(!isset($params['attrs']))
			$params['attrs'] = '';
    ...
    static public function textFieldsDynamicTable($name, $params = array('value' => '', 'attrs' => '', 'options' => array())) {
        ...
        if(!empty($params['options'])) {
            ...
            foreach($params['options'] as $key => $p) {
                $pattern[$key] = htmlNbs::text($name. '[]['. $key. ']');
            }
            ...
            elseif(!is_array($params['value'])) {
                $params['value'] = utilsNbs::jsonDecode($params['value']);
            ...
            do {
                ...
                foreach($params['options'] as $key => $p) {
                    ...
                    $res .= __($p['label']). htmlNbs::text($name. '['. $i. ']['. $key. ']', $paramsForText);
                }
                $res .= $remove. '</div>';
                $i++;
    ...
    static public function subscriptionList($name, $params = array('attrs'=>'', 'size'=> 5, 'options' => array(), 'value' => '')) {
      self::_loadSubscriptionListOptions();
      if(!empty(self::$subscriptionListOptions)) {
          $params['options'] = self::$subscriptionListOptions;
          $params['attrs'] .= ' type="subscriptionList"';
          return self::selectlist($name, $params);
      }
      return false;
    }
    static protected function _loadCategoriesOptions() {
        if(empty(self::$categoriesOptions)) {
            $categories = frameNbs::_()->getModule('products')->getCategories();
            if(!empty($categories)) {
    ...
    static protected function _loadProductsOptions() {
        if(empty(self::$productsOptions)) {
            $products = frameNbs::_()->getModule('products')->getModel()->get(array('getFields' => 'post.ID, post.post_title'));
            if(!empty($products)) {
    ...
    static protected function _loadSubscriptionListOptions() {
        if(empty(self::$subscriptionListOptions)) {
            $lists = frameNbs::_()->getModule('subscribers_lists')->getModel()->getSimpleList(array('getFields' => 'id, label'));
            if(!empty($lists)) {
                foreach($lists as $p) {
                    self::$subscriptionListOptions[$p['id']] = $p['label'];
                }
            }
        }
    }
	...
	static public function fontsList($name, $params = array('value' => '')) {
		...
		if(empty($options)) {	// Fill them only one time per loading
			$fontsList = isset($params['standard']) ? utilsNbs::getStandardFontsList() : array_merge(utilsNbs::getFontsList(), utilsNbs::getStandardFontsList());
		...
		if(isset($params['default'])) {
			$params['options'] = array(NBS_DEFAULT => $params['default']);
		}
	...
	static public function slideInput($name, $params = array('attrs' => '', 'checked' => false, 'id' => '')) {
		...
		return '<a class="toeSlideShellNbs" href="#"'. $params['attrs']. '>
			<span class="toeSlideButtNbs"></span>
			<span class="toeSlideOnNbs">'. __('ON'). '</span>
			<span class="toeSlideOffNbs">'. __('OFF'). '</span>
	...
	static public function galleryBtn($name, $params = array()) {
		...
		$buttonParams = $params;
		$buttonParams['value'] = isset($params['btnVal']) ? $params['btnVal'] : sprintf(__('Select %s', NBS_LANG_CODE), strFirstUp($galleryType));
	...
	/*static public function radialProgress($progress, $attrs = array()) {
		frameNbs::_()->addStyle('radial-progress', NBS_CSS_PATH. 'radial-progress.css');
	...
	static public function recaptcha($name, $params = array()) {
		frameNbs::_()->addScript('google.recaptcha', 'https://www.google.com/recaptcha/api.js?render=explicit&onload=nbsInitCaptcha');
		frameNbs::_()->addScript('google.recaptcha.frontend', NBS_JS_PATH. 'google.recaptcha.js');

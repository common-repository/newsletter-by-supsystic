...
class errorsNbs {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqNbs::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $nbsErrors = reqNbs::getVar('nbsErrors');
        if(!empty($nbsErrors)) {
            if(!is_array($nbsErrors)) {
                $nbsErrors = array( $nbsErrors );
            }
            $nbsErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $nbsErrors)));
            if(!empty($nbsErrors)) {
                self::$current = $nbsErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsNbs', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsNbs', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqNbs::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqNbs::clearVar('sesErrors', 'session');
    }

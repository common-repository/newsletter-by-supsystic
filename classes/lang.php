...
class langNbs {
	...
	static public function extract($d = array('dir' => '', 'langNbs' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langNbs']) && !empty($d['langNbs']))
			$langDirPath = NBS_LANG_DIR. $d['langNbs']. DS;
		else
			$langDirPath = NBS_LANG_DIR. NBS_WPLANG. DS;

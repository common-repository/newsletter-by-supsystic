...
class fieldAdapterNbs {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterNbs', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsNbs'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsNbs::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsNbs::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $brands = frameNbs::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', NBS_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', NBS_LANG_CODE). '</b><br />
                '. __('Destination', NBS_LANG_CODE). ':'. htmlNbs::checkboxlist('params[dest]', array('optionsNbs' => $listOfDest)). '<br />
                '. __('Country', NBS_LANG_CODE). ':'. htmlNbs::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', NBS_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', NBS_LANG_CODE) : htmlNbs::checkboxlist('params[categories][]', array('optionsNbs' => $cOptions))). '<br />
                    <b>'. __('Brands', NBS_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', NBS_LANG_CODE) : htmlNbs::checkboxlist('params[brands][]', array('optionsNbs' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', NBS_LANG_CODE).': '. htmlNbs::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', NBS_LANG_CODE).': '. htmlNbs::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', NBS_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameNbs::_()->getModule('optionsNbs')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlNbs::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', NBS_LANG_CODE).':<br />';
                $output .= htmlNbs::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', NBS_LANG_CODE).':<br />';
                $output .= htmlNbs::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsNbs']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsNbs'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', NBS_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsNbs'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameNbs::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameNbs::_()->getTable('states')
				->leftJoin( frameNbs::_()->getTable('countries'), 'country_id' )

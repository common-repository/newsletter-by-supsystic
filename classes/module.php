<?php
abstract class moduleNbs extends baseObjectNbs {
	...
	public function __construct($d/*, $params = array()*/) {
		...
		if(isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsNbs::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsNbs::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if(empty($this->_externalDir)) {
			return NBS_MODULES_DIR. $this->getCode(). DS;
		} else {
	...
	public function getModPath() {
		if(empty($this->_externalPath)) {
			return NBS_MODULES_PATH. $this->getCode(). '/';
		} else {
	...
	protected function _createController() {
		...
		if(file_exists($this->getModDir(). 'controller.php')) {
			$className = '';
			if(importNbs($this->getModDir(). 'controller.php')) {
				$className = toeGetClassNameNbs($this->getCode(). 'Controller');
			}
			if(!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir().'helper.php')) {
			...
			importClassNbs($helper, $this->getModDir(). 'helper.php');
	...
	public function addAdminScript( $handleKey = '', $path = '' ) {
		...
		frameNbs::_()->addScript( $handle, $path );
	}
	public function addAdminJSVar( $name, $val, $handleKey = '' ) {
		$handle = $this->_makeAssetHandle( 'admin', $handleKey );
		frameNbs::_()->addJSVar( $handle, $name, $val);
	}
	public function addAdminStyle( $handleKey = '', $path = '' ) {
		...
		frameNbs::_()->addStyle( $handle, $path );
	}
}

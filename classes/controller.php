...
abstract class controllerNbs {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameNbs::_()->getModule( $this->getCode() );
		$className = '';
		if(importNbs($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameNbs($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importNbs($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameNbs($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(NBS_ADMIN),
				'remove' => array(NBS_ADMIN),
				'restore' => NBS_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - NBS_METHODS or NBS_USERLEVELS
	 */
	...
	public function getModule() {
		return frameNbs::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) sanitize_text_field(reqNbs::getVar('page'));
		$rowsLimit = (int) sanitize_text_field(reqNbs::getVar('rows'));
		$orderBy = sanitize_text_field(reqNbs::getVar('sidx'));
		$sortOrder = sanitize_text_field(reqNbs::getVar('sord'));

		// Our custom search
		$search = reqNbs::getVar('search');
		...
		// jqGrid search
		$isSearch = sanitize_text_field(reqNbs::getVar('_search'));
		if($isSearch) {
			$searchField = sanitize_text_field(trim(reqNbs::getVar('searchField')));
			$searchString = sanitize_text_field(trim(reqNbs::getVar('searchString')));
		...
		$res = dispatcherNbs::applyFilters($this->getCode(). '_getListForTblResults', $res);
		$res->ajaxExec();
	}
	public function removeGroup() {
		$res = new responseNbs();
		if($this->getModel()->removeGroup(reqNbs::getVar('listIds', 'post'))) {

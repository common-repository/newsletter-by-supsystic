...
class utilsNbs {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsNbs::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsNbs::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameNbs::_()->getModule($m))
                return false;
            if(!is_dir(frameNbs::_()->getModule($m)->getModDir()))
                return false;
        }
        return true;*/
        foreach(frameNbs::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqNbs::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse wornbsess post/page/custom post type content for images and return it's IDs if there are images
	...
	static public function getReferalUrl() {
		// Simple for now
		return reqNbs::getVar('HTTP_REFERER', 'server');
	}
	...
	static public function setElementsStyle($content, $xPathQuery, $styles) {
		...
		foreach($elements as $element) {
			...
			$currentStyleEntries = array_filter(explode(';', $element->hasAttribute('style') ? $element->getAttribute('style') : ''), 'utilsNbs::_notEmpty');

			$currentStyles = empty($currentStyleEntries) ? array() : array_combine(
				array_map('utilsNbs::_keyBeforeColon', $currentStyleEntries),
				array_map('utilsNbs::_valueAfterColon', $currentStyleEntries)
			);
			$currentStyles = array_merge($currentStyles, $styles);
			$currentStyleEntries = array_map('utilsNbs::_implodeByColon', array_keys($currentStyles), $currentStyles);

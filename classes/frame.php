...
class frameNbs {
    ...
    public function __construct() {
        $this->_res = toeCreateObjNbs('response', array());

    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new frameNbs();
        }
        return $instance;
    }
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsNbs::getExtModDir( $m['ex_plug_dir'] );
                }
                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassNbs($code. strFirstUp(NBS_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameNbs($code);
    ...
    public function init() {
		...
		dispatcherNbs::doAction('afterModulesInit');

		modInstallerNbs::checkActivationMessages();
        ...
        register_activation_hook(  NBS_DIR. DS. NBS_MAIN_FILE, array('utilsNbs', 'activatePlugin')  ); //See classes/install.php file
        register_uninstall_hook(NBS_DIR. DS. NBS_MAIN_FILE, array('utilsNbs', 'deletePlugin'));
		register_deactivation_hook(NBS_DIR. DS. NBS_MAIN_FILE, array( 'utilsNbs', 'deactivatePlugin' ) );

		add_action('init', array($this, 'connectLang'));
        //$operationTime = microtime(true) - $startTime;
    }
	public function connectLang() {
		load_plugin_textdomain(NBS_LANG_CODE, false, NBS_PLUG_NAME. '/languages/');
	}
    /**
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', NBS_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[NBS_METHODS])
                    && !empty($permissions[NBS_METHODS])
                ) {
                    foreach($permissions[NBS_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[NBS_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[NBS_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameNbs::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ NBS_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ NBS_METHODS ][ $action ]))
                            || (!is_array($permissions[ NBS_METHODS ][ $action ]) && $permissions[NBS_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[NBS_USERLEVELS])
                    && !empty($permissions[NBS_USERLEVELS])
                ) {
                    $currentUserPosition = frameNbs::_()->getModule('user')->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if(is_multisite() && is_admin() && is_super_admin()) {
						$currentUserPosition = NBS_ADMIN;
					}
                    foreach($permissions[NBS_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
    ...
    protected function _extractTables($tablesDir = NBS_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = NBS_TABLES_DIR) {
        importClassNbs('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableNbs::_($tableName);
    }
    /**
     ...
     * @return object table
     * @example frameNbs::_()->getTable('products')->getAll()
     */
    ...
    public function addScript($handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array()) {
		...
		$src = empty($src) ? $src : uriNbs::_($src);
		if(!$ver)
			$ver = NBS_VERSION;
    ...
    /**
     * Add all scripts from _scripts array to wornbsess
     */
    ...
    public function isTplEditor() {
        $tplEditor = reqNbs::getVar('tplEditor');
	...
	public function isAdminPlugOptsPage() {
		$page = reqNbs::getVar('page');
		if(is_admin() && strpos($page, frameNbs::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
	...
	public function savePluginActivationErrors() {
		update_option(NBS_CODE. '_plugin_activation_errors',  ob_get_contents());
	}
	public function getActivationErrors() {
		return get_option(NBS_CODE. '_plugin_activation_errors');
	}
